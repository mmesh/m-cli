...
func (api *API) Show(i *billing.Invoice) {
	...
	t.AddRow(colors.Black("Customer Email"), colors.DarkWhite(i.CustomerEmail))
	...
	t.AddRow(colors.Black("Status"), invoiceStatus(i.Status))

	t.AddRow("", "")
	invItems := fmt.Sprintf("%d", len(i.Items))
	...
	if input.GetConfirm("Show invoice items?", false) {
		fmt.Println()
...
func showItem(ii *billing.InvoiceItem, currency string, n int) {
	t := table.New()
